...
public class ShoppingCartController {
    ...
    public R<ShoppingCart> add(@RequestBody ShoppingCart shoppingCart){
        ...
        ShoppingCart shoppingCartByUserId = shoppingCartService.getOne(queryWrapper);

        if(shoppingCartByUserId != null){
            //如果已经存在，就在原来数量基础上加一
            Integer number = shoppingCartByUserId.getNumber();
            shoppingCartByUserId.setNumber(number + 1);
            shoppingCartService.updateById(shoppingCartByUserId);
        }else{
            //如果不存在，则添加到购物车，数量默认就是一
            shoppingCart.setNumber(1);
            shoppingCart.setCreateTime(LocalDateTime.now());
            shoppingCartService.save(shoppingCart);
            shoppingCartByUserId = shoppingCart;
        }

        return R.success(shoppingCartByUserId);
    }
